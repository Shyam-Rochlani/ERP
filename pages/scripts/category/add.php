...
if(isset($_POST['add_category'])){
    ...
    $result_gst = mysqli_query($connection, $query);
    checkQueryResult($result_gst);
    
    if(mysqli_num_rows($result)==0 && mysqli_num_rows($result_gst) == 0){
        $query = "INSERT INTO category(category_name, hsn_code, created_by)
        VALUES('$category_name',$hsn_code,$employee_id)";
        
        $add_category_query_result = mysqli_query($connection,$query);
        
        checkQueryResult($add_category_query_result);
        
        $query = "INSERT INTO gst(hsn_code,gst_rate,wef,created_by)
        VALUES($hsn_code, $gst_rate, now(), $employee_id)";
        ...
        if($row['gst_rate']!=$gst_rate){
            ...
            checQueryResult($add_category_query_result);
        }
    }
        else{
