...
HAVING category.deleted=0";

if(isset($_POST["search"]["value"])){
    $query .= " AND (category.category_name like '%".$_POST["search"]["value"].
        "%' OR category.hsn_code like '%". $_POST['search']['value']."%')";
}
...
while($row = mysqli_fetch_assoc($result)){
    ...
    $sub_array[] = "<button class ='edit fa fa-pencil btn blue'
     id='".$row['   category_id']."' data-toggle='modal' data-target='#editModal'></button>";
    $sub_array[] = "<button class = 'delete fa fa-trash btn red' id='".
        $row['category_id']."' data-toggle='modal' data-target='#deleteModal'></button>";    
